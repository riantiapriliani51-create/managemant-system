...
# ================= EXPORT EXCEL PER RUANG (UNTUK LOGIN USER) =================
@app.route('/export-per-ruang/<ruang>')
def export_per_ruang(ruang):
    """Export Excel untuk ruang tertentu (Hanya untuk user/admin terdaftar)"""
    if session.get('role') not in ['admin', 'user']:
        return redirect(url_for('dashboard'))
    ...
    # ================= HEADER =================
    ws['A1'] = f'INVENTORI LAB LISTRIK - RUANG {ruang.upper()}'
    ws['A1'].font = Font(bold=True, size=14)
    ws.merge_cells('A1:E1')
